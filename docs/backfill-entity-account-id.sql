...
SELECT DISTINCT
  ...
  CASE
    WHEN @updatedColumn IS NULL THEN N' NULL AS SORT_DTTM,'
    ELSE N' eam.' + QUOTENAME(@updatedColumn) + N' AS SORT_DTTM,'
  END + N'
...
FROM (
  SELECT
    ...
    ROW_NUMBER() OVER (
      ...
      ORDER BY COALESCE(mb.SORT_DTTM, '19000101') DESC, mb.ENTITY_ACCOUNT_ID ASC
    ) AS rn
  FROM #MappingBase mb
  INNER JOIN #MappingAgg ma
